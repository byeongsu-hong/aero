...
func developerGenesisBlock(period uint64, faucet common.Address) *core.Genesis {
	...
	config.ChainID = big.NewInt(defaultNetworkId)
...
func setupNode(ctx *cli.Context) (*node.Node, error) {
	...
	if len(genesisPath) == 0 {
		...
		log.Println("Auto-generated faucet address : ", key.Address.Hex())
		log.Println(" ↳ Private Key : ", hexutil.Encode(crypto.FromECDSA(key.PrivateKey)))

		// make generated account default
		faucet, err = ks.ImportECDSA(key.PrivateKey, "")
		if err != nil {
			return nil, fmt.Errorf("Failed to import account: %v", err)
		}

	} else {
		// load genesis.json, created by puppeth
	...
	ethConfig.MinerGasPrice = big.NewInt(0)
